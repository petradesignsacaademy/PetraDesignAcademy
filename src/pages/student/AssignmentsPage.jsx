        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 24, background: 'var(--surface)', borderRadius: 12, padding: 4, width: 'fit-content' }}>
          {tabs.map(t => (
            <button key={t.key} onClick={() => setActiveTab(t.key)} style={{
              background: activeTab === t.key ? 'var(--purple)' : 'transparent',
        ...
        {filtered.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {filtered.map(sub => {
              ...
              return (
                <div key={sub.id} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 20, padding: '24px 28px', transition: 'box-shadow 0.2s' }}
                ...
                >
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12, gap: 16, flexWrap: 'wrap' }}>
                    <div>
                      <div style={{ fontFamily: 'Poppins, sans-serif', fontSize: 11, fontWeight: 700, letterSpacing: 1.5, color: 'var(--text3)', marginBottom: 4 }}>
                        {sub.lessons?.modules?.title?.toUpperCase()}
                      </div>
                      <h3 style={{ fontFamily: 'Cormorant Upright, serif', fontSize: 22, fontWeight: 700, color: 'var(--text)' }}>{sub.lessons?.title}</h3>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
                      {sub.star_rating && (
                        <div style={{ display: 'flex', gap: 2 }}>
                          {[1,2,3,4,5].map(s => (
                            <span key={s} style={{ fontSize: 14, color: s <= sub.star_rating ? '#F9A534' : 'var(--bg3)' }}>★</span>
                          ))}
                        </div>
                      )}
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
                    ...
                    <button onClick={() => {
                      const mod = sub.lessons?.modules
                      if (mod) navigate(`/courses/${mod.course_id}/modules/${mod.id}/lessons/${sub.lessons.id}`)
